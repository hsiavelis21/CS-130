...
s1 = sheets.Spreadsheet('Test1')
s2 = sheets.Spreadsheet('Test2')
...
def test_set_and_get_spreadsheet_cell_contents():
    ...
    with pytest.raises(ValueError):
        s2.set_spreadsheet_cell_contents(invalid_index1, "'hi", None)
        s2.set_spreadsheet_cell_contents(invalid_index2, "'hi", None)
        s2.set_spreadsheet_cell_contents('testing!!', "", None)
        s2.set_spreadsheet_cell_contents('', "", None)

    # CASE 2: Cell at specified location has empty contents
    s2.set_spreadsheet_cell_contents('A1', "'test", None)
    assert(s2.get_extent() == (1,1))
    
    assert(len(s2.dict) == 1)

    s2.set_spreadsheet_cell_contents('Z20', "1.234", None)
    assert(s2.get_extent() == (26, 20))

    # CASE 1: Cell at specified location has non-empty contents
    s2.set_spreadsheet_cell_contents('A1', "'new", None)
    ...
    s2.set_spreadsheet_cell_contents('CDF500', '', None)
    assert(s2.get_extent() == (26, 20))
    assert(len(s2.dict) == 2)

    # CASE 1.1: New cell has empty contents
    s2.set_spreadsheet_cell_contents('B2', '1.3.', None)
    assert(s2.dict['B2'].get_cell_type() == 'STRING')
    s2.set_spreadsheet_cell_contents('B2', '', None)
    assert(len(s2.dict) == 2)
    assert(s2.get_spreadsheet_cell_contents('B3') is None)
    s2.set_spreadsheet_cell_contents('Z20', '    ', None)
    print(s2.dict.keys())
    assert(s2.get_extent() == (1,1))
